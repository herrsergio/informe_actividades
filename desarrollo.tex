...
\begin{figure}[htb]
 \begin{center}
  \includegraphics{acceleport_xem_lg.png}
 \end{center}
...
\begin{center}
  \begin{tabular}{|c|c|c|c|c|}
   \hline
   Pin desde RJ-45 & Señal & Dirección & Señal & A pin DB-9
   \hline
   3 & GND (Tierra) & <-> & GND (Tierra) & Carcaza
   \hline
   4 & TxD (Transmit Data) & -> & RxD (Receive Data) & 2
   \hline
   5 & RxD (Receive Data) & <- & TxD (Transmit Data) & 3
   \hline
   6 & SG (Signal Ground) & <-> & SG (Signal Ground) & 5
   \hline
   7 & CTS (Clear to Send) & <- & DTR (Data Terminal Ready) & 4
   \hline
   1 & DCD (Data Carrier Detect) & <- & RTS (Request to Send) & 7
   \hline
   2 & RTS (Request to Send) & -> & CTS (Clear to Send) & 8
   \hline
   8 & DTR (Data Terminal Ready) & -> & DSR (Data Set Ready) & 6
   \hline
  \end{tabular}
\end{center}
...
Se muestra dos tickets, el número 335 y 336, lo que indica que provienen de la caja 3. El \#335 es un pedido para llevar y el #336 es para comer en el restaurante. Se muestran las cantidades de los productos, su precio, el total, el efectivo con que se pagó y el cambio, el número de ticket único, así como la fecha y hora. El ticket \#335 que se manda a imprimir para darselo al cliente se muestra a continuación:
...
Está es la descripción de la tabla de PostgreSQL encargada de almacenar los usos ideales:
...
La llave foránea \texttt{inv_id} pertenece a la tabla \texttt{op_grl_cat_inventory}, la cual contiene, entre otros datos, el nombre del producto y la unidad de medida.

El los restaurantes existe una aplicación llamada \textit{e-Reports}, la cual está desarrollada en JSP\footnote{Java Server Pages}. En está aplicación se añaden los reportes requeridos clasificandolos de acuerdo a su finalidad. El reporte de ``Usos Ideales'' se  encuentra en el menú de \textit{Mano de Obra -> Planeación -> Usos Ideales Diarios}.
...
\begin{itemize}
 ...
 \item Home_Delivery.conf
 \item Pure_Ensalada.conf  
 \item Servicio.conf
\end{itemize}
...
El reporte obtiene de la tabla \texttt{op_gt_real_sist_mng} el pronóstico de número de transacciones y las transacciones reales más cercanas a la fecha seleccionada.
